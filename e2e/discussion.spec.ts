...
test.describe("Discussion API - auth guard", () => {
  ...
  test("POST comment ditolak jika belum login", async ({ request }) => {
    const response = await request.post(
      api(`/api/v1/readings/${readingId}/comments`),
      {
        data: { content: "Test" },
      },
    );
...
test.describe.serial("Discussion backend functional flow", () => {
  ...
  test("GET readings untuk mendapat readingId (jika ada)", async ({
    request,
  }) => {
  ...
  test("POST /api/v1/readings/{id}/comments membuat komentar baru", async ({
    request,
  }) => {
    const response = await request.post(
      api(`/api/v1/readings/${readingId}/comments`),
  ...
  test("GET /api/v1/readings/{id}/comments memuat komentar", async ({
    request,
  }) => {
    const response = await request.get(
      api(`/api/v1/readings/${readingId}/comments?page=0&size=10`),
      {
        headers: authHeaders(),
      },
    );
    expect(response.ok()).toBeTruthy();
    const body = await response.json();
    const comments: { id: string }[] = Array.isArray(body)
      ? body
  ...
  test("POST /api/v1/readings/{id}/comments/{parentId}/replies membuat balasan", async ({
    request,
  }) => {
    const response = await request.post(
      api(`/api/v1/readings/${readingId}/comments/${commentId}/replies`),
      {
        headers: authHeaders(),
  ...
  test("PUT /api/v1/readings/{id}/comments/{commentId} mengupdate komentar", async ({
    request,
  }) => {
    const response = await request.put(
      api(`/api/v1/readings/${readingId}/comments/${commentId}`),
      {
        headers: authHeaders(),
  ...
  test("POST /api/v1/comments/{commentId}/reactions menambahkan reaksi", async ({
    request,
  }) => {
    const response = await request.post(
      api(`/api/v1/comments/${commentId}/reactions`),
      {
        headers: authHeaders(),
  ...
  test("DELETE /api/v1/comments/{commentId}/reactions menghapus reaksi", async ({
    request,
  }) => {
  ...
  test("DELETE /api/v1/readings/{id}/comments/{commentId} menghapus komentar", async ({
    request,
  }) => {
    const response = await request.delete(
      api(`/api/v1/readings/${readingId}/comments/${commentId}`),
